...
> Status: **as-built** through the windowed-model stack (PR5-C) **plus the VIEW_FULL-replica drop** (PR #3226, branch `gui/windowed-tx-table-drop-viewfull` off `development`). This is the GUI subsystem that renders the wallet's transaction history: the detailed `TransactionView` `QTableView` and the `OverviewPage` recent-transactions list. This file is the **as-built companion** to `doc/transaction_table_windowed_model.md` (the design-of-record / planning history, parts of which remain aspirational); every claim here is anchored to source. Where the two disagree, this file describes what the code actually does.
>
> **PR #3226 update (final piece).** The last full-wallet copy on the GUI side — the `VIEW_FULL` replica (`cachedWallet`) inside `TransactionTablePriv` — is gone. `TransactionTableModel` is now a stateless per-row **formatter** (`QObject`, not `QAbstractTableModel`); the two windowed views hold only their slice. The node-side bootstrap `WalletTxStore::reloadAndSnapshot()` (which *returned* an O(wallet) `List<TransactionRecord>`) became `prime()` returning **void** — the full list no longer crosses the `interfaces::` boundary. Sections below are updated in place; line-number anchors reflect the PR #3226 branch. Historic PR5-C references to `reloadAndSnapshot` / `TransactionTablePriv` / TTM-as-a-model have been reconciled to the current code.
...
```
   ...
   │  ┌──────────────┐                           │                    │   ├─ VIEW_FULL → BitcoinGUI balloon        │
   │  │ intake deque │  cs_intake                 │                    │   │     (RowsInserted→formatRole, no model)│
...
| The store-worker NEVER takes `cs_main`/`cs_wallet` | `insert/remove/updateTransaction` take only `cs_store`; this is what makes `prime`'s park-while-holding-both-wallet-locks deadlock-free |
...
The load-bearing exclusion argument for `prime` is that **`cs_main` is held at every producer of an insert/remove, including both `CT_DELETED` sites**. Two comments describe these sites, and they **disagree**:

- `prime`'s comment (`wallettxstore.cpp:390-399`) is **correct**: it states that both `CT_DELETED` fire sites — `ReorganizeChain` (`main.cpp`, `EXCLUSIVE_LOCKS_REQUIRED(cs_main)`, ~`main.cpp:1106`) and `ResendWalletTransactions` (`wallet.cpp`, `EXCLUSIVE_LOCKS_REQUIRED(cs_main)`, ~`wallet.cpp:2111`) — hold `cs_main` *even though they do not hold `cs_wallet`* at the fire site. That is exactly why holding `cs_main` across the rebuild + drain fully excludes producers.
...
- `m_limit_enabled`/`m_limit_time` (`:361-362`) — the OptionsModel datetime-display cutoff, set only by `prime()`. **Changing the cutoff is not cheap:** `OptionsModel::LimitTxnDisplayChanged` → `WalletModel::reloadTransactionView` (`walletmodel.cpp:260`, wired at `:55`) → `txSource().prime(...)` + `requestEventDrainSoon()` — a **full store rebuild under `LOCK2`** that re-decomposes the whole wallet and re-publishes every cursor `Reset` (same path as construction, below), then a prompt drain applies the Resets to the views.
...
### Prime (`prime`, `wallettxstore.cpp:386-499`)
Qt-thread full rebuild, returning **void** — the scanned records stay node-side in `m_records`; nothing is returned to the caller (PR #3226 dropped the O(wallet) `List<TransactionRecord>` return that `reloadAndSnapshot` used to marshal to the GUI). Holds `LOCK2(cs_main, cs_wallet)` (`:400`) across the *entire* rebuild **and** the queue drain — `cs_main` is the load-bearing exclusion lock because every producer of insert/remove holds it (both `CT_DELETED` sites hold `cs_main` even without `cs_wallet`; documented at `:390-399` — and see the §3 comment-contradiction warning). It quiesces the worker (`m_rebuilding` + waits on `m_idle_cv` for `m_worker_parked`), rescans `mapWallet` (`:419`), **decomposes + `updateStatus` + `populateDisplayLabel` each visible tx producer-side**, sorts, `std::move`s the scan buffer into `m_records` and swaps index/caches/volatile/cursors under `cs_store` (`:448`), drains+discards pre-rebuild events, then re-publishes per-view cursor Resets AFTER the drain (recording each into `m_view_seqno`, `:467-469`,`:488`), then releases the worker. Ordering is load-bearing: republishing the Resets *before* the drain would discard them.

This is the one **O(N)-under-both-wallet-locks** operation the rewrite does **not** eliminate — it keeps the node-side wallet scan; what PR #3226 removed is the *transfer* of the resulting list across the `interfaces::` boundary, not the scan. It is structurally the old `loadWallet` scan, and it runs (a) at construction — `WalletModel` ctor calls `txSource().prime(...)` (`walletmodel.cpp:45`) **before any windowed view registers**, so the views register against a populated store — and (b) on every datetime-cutoff toggle (above). It is the startup freeze §14 discloses; its deferred remediation is the design-of-record's "step 6" progressive background decompose (§12, §14), which is not yet implemented.
...
- **Park/quiesce barrier**: `prime` sets `m_rebuilding` and waits on `m_idle_cv` for the worker to park; the wait is gated on `m_started` so the first ctor-time prime (before `start()` launches the worker) skips it.
...
- Clears `m_event_drain_requested` up front, drains ≤ `MODEL_EVENT_DRAIN_MAX_BATCH=1024` (`guiconstants.h:21`), records the last `ChainTipChanged` height into `cachedNumBlocks` (`:189-193`), emits `transactionUpdated()` (`:215`, the general "wallet txns changed" pulse OverviewPage refreshes on), then emits `walletEventsDrained(events)` (`:219`) to fan the *same* batch to **all** windowed consumers (`OverviewTxModel`, `DetailedTxModel`) and the new-transaction balloon, then `checkBalanceChanged` + `numTransactionsChanged` once per batch, then re-arms via `QTimer::singleShot(0)` if the batch hit the cap. Post-PR #3226 there is no full-replica `applyEventBatch`/`updateConfirmations` step: per-row confirmation refresh is driven by each view's own handling of the producer's per-tip `RowsChanged` events (from `applyChainTipRefresh`), not a model-wide `dataChanged`.
...
### Stateless `TransactionTableModel` formatter (`transactiontablemodel.{h,cpp}`)
Post-PR #3226 the TTM is a **stateless per-row formatter** — a plain `QObject`, no longer a `QAbstractTableModel`, holding **no per-row state** (the `TransactionTablePriv` replica, `data()`/`index()`/`indexForTxid()` and the `applyEventBatch()`/`updateConfirmations()`/`updateDisplayUnit()` surface are all gone). It exists only to single-source, in one place, (a) the `ColumnIndex`/`RoleIndex` enums the windowed views key on, (b) the role/column rendering, and (c) the column set.

- `formatRole(const TransactionRecord* rec, int column, int role)` (`transactiontablemodel.cpp:451`) is the single role-rendering core; it takes a `const` record (the record may be *any* served row from any consumer, not owned by this object) and keeps no state, so `DetailedTxModel::data`, `OverviewTxModel::data`, `BitcoinGUI`'s balloon, and the CSV `ExportRowsModel` all call it directly (the const-ness removed the earlier `const_cast` shims). It validates the column range up front with an explicit guard (not an `assert`), so an out-of-range column degrades to an empty `QVariant` in any build.
- `columnCount()`/`headerData()` (`transactiontablemodel.cpp:47`,`:55`) are the shared column definitions; `DetailedTxModel` and the CSV export delegate to them so the column set lives in exactly one place.

Because no view binds the TTM as a model any more, it emits no `dataChanged`/`rowsInserted` and needs no confirmation/unit refresh of its own — those concerns moved to the windowed views (which drive off the producer event stream and, for the display unit, connect `OptionsModel::displayUnitChanged` directly; see §10 *Display unit*).
...
**Status.** Computed producer-side under the locks already held — `NotifyTransactionChanged` calls `updateStatus` (`walletmodel.cpp:836`) before enqueue, `prime` during its rescan (`wallettxstore.cpp:419+`), and per-block `applyChainTipRefresh` (`wallettxstore.cpp:591-642`) over the bounded volatile set. Since PR5-C deleted the lazy on-read `index()` refresh (and PR #3226 deleted the whole replica), the producer is the *sole* authoritative status source, not a head start. (What is admitted into the status-bearing record set in the first place is the producer-admission decision in §4 — including the transient-orphan-coinstake override.)

**Display unit.** The BTC/mBTC/… unit is read live by `formatRole` at render time, so a unit change needs only a repaint, not a re-fetch. Each live view wires it independently: `OverviewPage::updateDisplayUnit` (`overviewpage.cpp:481`) sets its delegate's unit and calls `listTransactions->update()`; `DetailedTxModel::updateDisplayUnit` (`detailedtxmodel.cpp`) re-emits `dataChanged` over the Amount column (one signal; Qt repaints only visible rows). Both connect to `OptionsModel::displayUnitChanged`. This replaces the old `TransactionTableModel::updateDisplayUnit` `dataChanged`, which was already observerless before PR #3226 (no view bound the TTM) — i.e. the detailed view's unit refresh is now wired for the first time here, closing a gap that predated this stack.
...
- **Up-channel** (consumer → producer): `getRows`/`getAllRows`/`rowForKey`/`getRowDetail`/`setViewport`-equivalents/`setSort`/`setFilter` return by value, and the bootstrap `prime(limit_enabled, limit_time)` returns **void**. The call *shape* is final; only the transport swap (the `interfaces::` shell + Cap'n Proto schema) is deferred (design-of-record lines 337-348). PR #3226 was specifically the step that made the boundary window-only: the old `reloadAndSnapshot` returned an O(wallet) `List<TransactionRecord>` — a full-history marshal on every load/toggle, exactly the anti-pattern the split must avoid — whereas `prime` triggers the node-side rebuild and the consumers pull only their window through `getRows`. The `interfaces::WalletTxSource` C++ contract and the Cap'n Proto schema (`src/ipc/capnp/wallet_tx_source.capnp`, `prime @10 (...) -> ()`) already reflect this.
...
| VIEW_FULL drop | "gui: windowed transaction views — drop the full-wallet snapshot" | `c0382e421` | #3226 (base `development`) | Delete the `VIEW_FULL` replica (`TransactionTablePriv`); TTM `QAbstractTableModel` → stateless `QObject` formatter; `reloadAndSnapshot` (returned O(wallet) list) → void `prime`; balloon via `walletEventsDrained`; overview click-through by `uint256`; wire `DetailedTxModel` display-unit refresh |
| **Step 6 (deferred, NOT landed)** | — | *no code yet* | — (design-of-record step 6, doc:247) | Off-Qt-thread **progressive startup decompose**: the O(N) decompose moves to a chunked background core task; the table opens at row count 0 and paints progressively. The deferred remediation for the startup freeze (§14). |
...
| `applyChainTipRefresh` runs INLINE on the validation thread | It needs `cs_wallet`; on the worker it would force the worker to take the wallet locks → deadlock `prime`'s park protocol | O(volatile × view_index) reposition cost lands on the validation thread per block |
...
- **Startup (and datetime-toggle) O(N) decompose under `LOCK2` is NOT eliminated.** `prime` (`wallettxstore.cpp:386-499`) still holds `LOCK2(cs_main, cs_wallet)` (`:400`) across the entire full-wallet rescan + `decomposeTransaction` + `updateStatus` + `populateDisplayLabel` rebuild. It runs at construction (`WalletModel` ctor, `walletmodel.cpp:45`, before any view registers) and on every datetime-cutoff change (`OptionsModel::LimitTxnDisplayChanged` → `WalletModel::reloadTransactionView` → `prime`, wired at `walletmodel.cpp:55`). This is exactly the startup freeze §1 flags for the old model — the rewrite reduces *per-block* and *render-path* cost, and (PR #3226) removes the full-list *transfer* across the boundary, but not this one-time/on-toggle node-side decompose.
...
- **In-code comment contradiction on the `CT_DELETED` lock state.** `walletmodel.cpp:764` wrongly says the `CT_DELETED` sites hold neither lock (with stale line numbers); `wallettxstore.cpp:390-399` correctly says they hold `cs_main`. The correct claim is load-bearing for `prime`'s exclusion (see §3). Fix the stale comment to avoid misleading a future editor.
- **Inaccurate `-DNDEBUG` rationale in several in-code comments.** Comments in `removeLocked` and the `formatRole` column guard justify their runtime checks by claiming the deployed build is `-DNDEBUG` (so `assert` is elided). That is wrong: the build system explicitly undefines `NDEBUG` (`-UNDEBUG` / `/U NDEBUG`, `CMakeLists.txt:54,56`), so `assert` is active in every configuration. The runtime guards are still correct and worth keeping — but for the right reason: they **degrade safely** (log + skip, never an out-of-bounds op or a GUI-aborting `assert`) if a producer ever violates the contiguity/position contract. Reword the comments so a future editor does not believe asserts are compiled out. (The old `applyRowsInserted`/`applyRowsRemoved` replica sites that also carried this comment were deleted with the VIEW_FULL replica in PR #3226.)
- **TTM is now a formatter, not a model — do not re-add a replica lightly.** PR #3226 removed the `VIEW_FULL` consumer entirely; the `TransactionTableModel` is a stateless `QObject`. If a future change needs a full-history model again, it must re-derive it from the producer event stream (not a paint-thread refresh) and re-introduce a `VIEW_FULL` refresh for in-place CT_UPDATED/CT_UPDATING status changes (which currently emit no `VIEW_FULL` event) — the same hazards the old replica carried, now with no scaffolding in place.
