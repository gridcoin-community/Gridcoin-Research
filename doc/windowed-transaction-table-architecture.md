...
- `removeLocked` (`:307-364`): resolve `[minPos, maxPos]`, then a **runtime structural guard** (`:336-347`) that bails (LogPrintf, no erase) if the run is non-contiguous/out-of-range — because the deployed build is `-DNDEBUG`, an assert would vanish and a wrong erase would swallow foreign rows.
...
| Runtime structural guard (LogPrintf + bail), not assert, in remove/update | `-DNDEBUG` drops asserts; a de-clustered erase would corrupt structure — a stale row is a safe degradation | An always-on runtime check; can silently leave a stale row in the (impossible) violated case |
