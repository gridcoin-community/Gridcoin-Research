...
Because there is no ability to login to the
...
using apt-get to install build dependencies, except for the Linux 64bit build. 
This guarantees that the tester is using the same versions as Gitian, so the 
build results are nearly identical to what would be found in a final release. 
However, this also means that builds will fail if new dependencies are 
introduced without being added to the dependency generator.
...
Gridcoin committer via the Travis web interface.
