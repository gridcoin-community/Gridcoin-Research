...
- **Tier 2 — `dm-flakey` device-mapper fault injection**. Wraps the wallet's testnet datadir in a loopback-backed ext4 volume layered under a dm-flakey device-mapper target, with `drop_writes` or `corrupt_bio_byte` features active during configurable down intervals. Linux-only, requires root. The harness exercises the *whole stack* (BDB wallet, LevelDB block index, blk\*.dat block files, ext4 filesystem) under random in-flight write loss/corruption. **What this actually validates is the layered defence below Phase 2** — see the post-merge note at the bottom of "Bugs caught by isolated-testnet validation". `drop_writes` mode demonstrates Phase 1's barrier ordering holding (the missing half is always the LevelDB-tip half, not the blk\*.dat half, so LevelDB rolls back one block and Phase 2's coherence check passes without rewind). `corrupt_bio_byte` mode demonstrates that random whole-bio corruption is caught by BDB and LevelDB integrity checks (DB_RUNRECOVERY, MANIFEST CRC) before reaching Phase 2's narrow trigger condition. Tier 2 does *not* reliably exercise Phase 2's rewind code path itself — that is what Tier 3 is for.
- **Tier 3 — deterministic `blk*.dat` tail truncation**. Scripted procedure (`tier3_truncate.sh`). Stop the wallet cleanly so the active `blk*.dat` is closed. Truncate the file tail by a configurable amount (default 64 KiB — picks off roughly 80 blocks at typical Gridcoin densities). Restart and observe Phase 2: the backward walk fails on the truncated tip, walks back until it finds a coherent block, runs the surgical `CleanAbandonedRange` + `PurgeOrphanedBlockIndexEntries` cleanup, then P2P re-supplies the truncated content from peers. After the chain re-syncs, the test does a **second** clean stop/restart and verifies Phase 2 reports "no rewind needed" — proving the durability invariant. Replaces an earlier draft that used VMware abrupt-power-off, which was hard to automate and only validated detection. Truncation gives the same on-disk symptom (block read fails) with the added benefit of being deterministic and parameterizable. **This is the harness that actually exercises Phase 2's rewind path end-to-end** — and the one most worth running before merge and after any change to Phase 2 code. The `TRUNCATE_BYTES` parameter sweeps from a few hundred bytes (Tier 1 sub-case, no SB cross) through ~1 MB (Tier 2 sub-case, 1–2 SBs crossed) up to ~4 MB+ (Tier 3 sub-case, 6+ SBs crossed) to cover every fork in the recovery code (Reset vs clamp, `DropAboveHeight`, side-chain catch-up).
...
## Lessons from Tier 2 fault-injection validation

Tier 2 (`tier2_dmflakey.sh`) was originally pitched as "the script most worth running before merge" — the idea being that dm-flakey reproduces real-world disk failure more faithfully than the surgical Tier 3 truncation. Two end-to-end Tier 2 runs against the post-fix branch (Iteration 9a `drop_writes` and 9b `corrupt_bio_byte`, recorded in `block_corruption_recovery_test_log.md`) demonstrated that this framing was wrong, and *why* it was wrong is itself part of the design's value:

1. **Tier 2 hits every layer of the stack, not just blk\*.dat.** dm-flakey wraps the whole datadir volume, so `drop_writes` discards (and `corrupt_bio_byte` mangles) writes for the BDB wallet log, the BDB environment files, the LevelDB MANIFEST, LevelDB SSTs, the LevelDB WAL, *and* `blk*.dat` — indiscriminately. Each of those layers has its own integrity check, and each fires before control reaches Phase 2.

2. **The lower-layer checks beat Phase 2 to the punch in two of the three observed outcomes.** In Iter 9a, Phase 1's `FileCommit(blk*.dat)` → `CTxDB::Sync()` ordering meant the LevelDB-tip commit was the half lost when drops landed at the wrong moment, so LevelDB rolled back one block cleanly and Phase 2's coherence check reported "no rewind needed." In Iter 9b, BDB's `DB_RUNRECOVERY` and (after `wallet.dat` + `database/` were restored from snapshot) LevelDB's MANIFEST CRC each fired before any block-index code ran at all.

3. **Phase 2's trigger condition is narrow on purpose.** It activates only when LevelDB is healthy enough to load the block index *and* the on-disk `blk*.dat` data at the recorded tip position hashes to something other than the `CBlockIndex` hash. That pattern is what Tier 3's surgical `truncate(1)` produces deterministically; it is *not* what random whole-bio corruption tends to produce, because LevelDB's own per-record checksums catch corruption that lands in its WAL or SST files first.

4. **The pre-merge recommendation is now Tier 3, not Tier 2.** Tier 3 parametrically sweeps `TRUNCATE_BYTES` to cover the small-truncation (no SB), mid-size (1–2 SBs crossed → clamp path), and large (6+ SBs crossed → `Reset` + `DropAboveHeight` + side-chain catch-up) sub-cases. That coverage is what locks down the Phase 2 *recovery code*. Tier 2 remains useful as a sanity check on the Phase 1 invariant and the lower-layer integrity checks — but the design intent of "validate Phase 2 before merge" is what Tier 3 actually delivers.

5. **Both Tier 2 sub-iterations passed in the "graceful failure" sense.** Iter 9a auto-recovered (LevelDB rolled back, P2P resynced); Iter 9b stopped with clear operator-actionable error messages (BDB's `To recover, BACKUP THAT DIRECTORY, then remove everything from it except for wallet.dat.` and LevelDB's `Corruption: checksum mismatch`). Neither resulted in a silently-advancing corrupt chain. The layered defence held.

This is itself a real result: it shows that the choice of harness has to match the trigger condition of the code under test. Tier 2 stresses the whole stack and verifies no layer silently advances; Tier 3 stresses Phase 2 in particular and verifies its recovery code paths. Both belong in the harness directory, but only one of them is what to run before merging a Phase 2 change.
