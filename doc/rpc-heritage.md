...
| `getmempoolentry` | src/rpc/mempool.cpp | `(txid)` | `{beacon_cpid, contract_types, fee, feerate, height, mrc_cpid, mrc_fee, size, time, unbroadcast}` | `961d91ecced1` | Adds Gridcoin `contract_types`/`mrc_cpid`/`mrc_fee`/`beacon_cpid` fields and drops upstream's ancestor/descendant/RBF/witness surface. `unbroadcast` matches upstream's per-entry field of the same name and type (Core `entryToJSON`, added a7ebe48b94); upstream additionally surfaces it via getmempoolancestors/getmempooldescendants, which Gridcoin does not implement. |
| `getmempoolinfo` | src/rpc/mempool.cpp | `()` | `{beacon_count, bytes, mandatory_sidestake_count, maxmempool, mrc_count, size, unbroadcastcount, usage}` | `218a7675dffa` | Adds Gridcoin `mrc_count`/`beacon_count`/`mandatory_sidestake_count` and omits upstream fee/relay fields. `unbroadcastcount` matches upstream's field of the same name. |
...
| `getrawmempool` | src/rpc/mempool.cpp | `(verbose)` | `{beacon_cpid, contract_types, fee, feerate, height, mrc_cpid, mrc_fee, size, time, unbroadcast}` | `3c50ec109dbb` | Non-verbose path returns the txid array; the verbose path (added #3029, impl moved to rpc/mempool.cpp) renders MempoolEntryToJSON with Gridcoin contract_types/mrc_cpid/mrc_fee/beacon_cpid fields and drops upstream's ancestor/descendant/RBF/witness surface and the 'mempool_sequence' arg + machinery. A porter must not blind-take upstream verbose/sequence changes. The verbose path's `unbroadcast` matches upstream's per-entry field of the same name and type (Core `entryToJSON`, added a7ebe48b94). |
...
| `getreceivedbyaddress` | src/wallet/rpcwallet.cpp | `(address, minconf)` | `{}` | `abf78a6d1f07` | GRC excludes coinbase outputs entirely (upstream counts mature coinbase) but DOES tally PoS coinstake receipts, netted against the staked principal via the shared TallyTxReceipts helper — a coinstake returns principal as well as minting reward, so face-value counting would report principal as income; immature generated value is excluded, matching upstream's 'include_immature_coinbase' default, but the arg itself is still absent (upstream delegates to a GetReceived helper). GRC also matches outputs by destination rather than by raw scriptPubKey, which upstream does not need: every GRC coinstake output is P2PK even when the staked UTXO was P2PKH. A porter must preserve the coinstake netting and the coinbase exclusion, and account for the missing immature-coinbase option. |
| `getreceivedbylabel` | src/wallet/rpcwallet.cpp | `(label, minconf)` | `{}` | `7c0f5e283380` | Label twin of getreceivedbyaccount (the 0.17 replacement Gridcoin adopted late); same tally divergences as getreceivedbyaddress (excludes coinbase, tallies coinstake receipts net of the staked principal, excludes immature generated value, no 'include_immature_coinbase' arg) and resolves the label via the shared address-book name rather than upstream's CWallet::GetLabelAddresses. |
...
| `listreceivedbyaddress` | src/wallet/rpcwallet.cpp | `(minconf, includeempty, includeWatchonly)` | `{account, address, amount, confirmations, involvesWatchonly, txids}` | `4b8fb0ed2cee` | Gridcoin's ListReceived retains the deprecated accounts subsystem (emits 'account' + 'involvesWatchonly' rather than upstream's 'label'), skips coinbase but tallies coinstake receipts net of the staked principal, treats minted value as external so an unbooked staking or sidestake address still gets a row, and lacks upstream's newer 'address_filter' and 'include_immature_coinbase' args (immature generated value is excluded, matching that arg's default); a porter must remap account->label semantics, preserve the coinstake netting, and not assume the address_filter/immature-coinbase params exist. |
| `listreceivedbylabel` | src/wallet/rpcwallet.cpp | `(minconf, includeempty, includeWatchonly)` | `{account, address, amount, confirmations, involvesWatchonly, txids}` | `4b8fb0ed2cee` | Shares Gridcoin's ListReceived helper with listreceivedbyaddress/listreceivedbyaccount (hence the identical fingerprint; the actual grouped rows carry 'label' last via a runtime group key the literal scan cannot see), skips coinbase but tallies coinstake receipts net of the staked principal, treats minted value as external so an unbooked staking or sidestake address still rolls into a group, and lacks upstream's 'address_filter'/'include_immature_coinbase' args (immature generated value is excluded, matching that arg's default); a porter must not assume upstream's GetReceived/ListReceived split. |
...
| `sendtoaddress` | src/wallet/rpcwallet.cpp | `(address, amount, comment, comment_to, message)` | `{}` | `ce89abdb7a10` | Gridcoin adds a Gridcoin-only 5th arg 'message' that attaches a GRC::TxMessage contract to the transaction (accepted but DISCARDED once the network disables MESSAGE contracts, the payment still being sent and a warning logged; the arg is retained deliberately so existing callers keep working) and otherwise keeps the legacy SendMoneyToDestination path; it lacks all of upstream's subtractfeefromamount/replaceable/conf_target/estimate_mode/avoid_reuse/fee_rate/verbose args and the verbose OBJ result, so a porter must preserve the contract-message arg and cannot map positions to upstream. |
...
| `walletpassphrase` | src/wallet/rpcwallet.cpp | `(passphrase, timeout, stakingonly)` | `{}` | `2c3e2b290e76` | Gridcoin adds a third PoS-only `stakingonly` arg that unlocks with the ppcoin staking-only scope (`UnlockScope::StakingOnly`, set with the master key rather than a separate flag; a lock clears it) and rejects re-unlock with RPC_WALLET_ALREADY_UNLOCKED, and relocks by arming a one-shot scheduler task validated against the unlock's epoch, so a superseded relock is a no-op, rather than upstream's scheduler weak-ptr callback; a porter must preserve the stakingonly arg/scope and that epoch check, and not blindly adopt upstream's multiwallet relock model. |
...
| `cancelunbroadcasttransaction` | src/wallet/rpcwallet.cpp |
...
| `splitunspent` | src/rpc/rawtransaction.cpp |
