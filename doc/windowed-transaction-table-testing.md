...
| **macOS ARM64 / Intel** | `cmake_production.yml:314` / `:701` (ARM64 on `macos-14`; Intel on the native `macos-15-intel` runner — no Rosetta) | `ENABLE_GUI=ON` (`:426`/`:816`), `ENABLE_TESTS=ON` (`:431`/`:821`), `Release`, **AppleClang** | `ctest --test-dir build` (`:451` / `:838`) → both suites | The only **macOS / AppleClang** runs — a second non-GCC toolchain over both binaries on two architectures. |
