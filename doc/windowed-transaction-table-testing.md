...
| **macOS ARM64 / Intel** | `cmake_production.yml`, jobs `macos-native-arm64` / `macos-native-intel` (ARM64 on `macos-14`; Intel on the native `macos-15-intel` runner — no Rosetta) | `ENABLE_GUI=ON`, `ENABLE_TESTS=ON`, `Release`, **AppleClang** (see each job's `Configure CMake` step) | `ctest --test-dir build` in each job's `Test` step → both suites | The only **macOS / AppleClang** runs — a second non-GCC toolchain over both binaries on two architectures. |
