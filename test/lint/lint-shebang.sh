...
export LC_ALL=C
EXIT_CODE=0
for PYTHON_FILE in $(git ls-files -- "*.py" ":(exclude)src/bdb53/" ":(exclude)src/ipc/libmultiprocess/"); do
...
for SHELL_FILE in $(git ls-files -- "*.sh" ":(exclude)src/bdb53/" ":(exclude)src/ipc/libmultiprocess/"); do
