...
# Drop the inactive sibling packaging dir from the source tree before dpkg-source.
# The orig tarball already excludes it (above); dpkg-source -b must not see it either,
# or it reports the variant's files (incl. a binary .png with no include-binaries
# entry) as unrepresentable upstream changes and aborts. This makes the mainnet build
# symmetric with the testnet path, where the caller has already renamed it into debian/.
rm -rf "$WORK/${PREFIX}/debian-testnet"
