...
Module modWinAPI
 ...
 As Boolean
    ...
    'See: System.Windows.Forms.UnSafeNativeMethods.etWindowLong*
    <DllImport("user32.dll")> _
    ...
    Public Sub RemoveTitleBar(ByVal handle As IntPtr)
        ...
        If hMenu <> IntPtr.Zero Then
            ...
            If n > 0 Then
                ...
                '   RemoveMenu(hMenu, CUInt(n), MF_REMOVE)

                '  RemoveMenu(hMenu, CUInt(n + 1), MF_REMOVE)
    ...
    Private Sub CreateBatchFile()
        Dim sPath As String
        sPath = "start cmd /k $path\gridcoin\cgminer.exe"
        Dim sCap As String
        sCap = "$path\cmd.exe " + sPath + " "
        Dim oSR As New StreamWriter("c:\out1.bat", False)
        oSR.WriteLine("color 1a")
        oSR.WriteLine("cd $\cgminergridcoin")
        oSR.WriteLine("cmd /k cgminer.exe -c cgminer0.conf")
        oSR.Close()
    End Sub
