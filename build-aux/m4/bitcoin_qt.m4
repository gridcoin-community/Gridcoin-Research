...
AC_DEFUN([_BITCOIN_QT_FIND_STATIC_PLUGINS],[
    if test "x$qt_plugin_path" != x; then
      ...
      PKG_CHECK_MODULES([QTCONCURRENT], [Qt5Concurrent${qt_lib_suffix}], [QT_LIBS="-lQt5Concurrent${qt_lib_suffix} $QT_LIBS"])
      dnl Gridcoin uses Charts:
      PKG_CHECK_MODULES([QTCHARTS], [Qt5Charts${qt_lib_suffix}], [QT_LIBS="-lQt5Charts${qt_lib_suffix} $QT_LIBS"])
      dnl Gridcoin uses SVG:
      PKG_CHECK_MODULES([QTSVG], [Qt5Svg${qt_lib_suffix}], [QT_LIBS="-lQt5Svg${qt_lib_suffix} $QT_LIBS"])
...
AC_DEFUN([_BITCOIN_QT_FIND_LIBS],[
  ...
  dnl Gridcoin uses Charts:
  BITCOIN_QT_CHECK([
    PKG_CHECK_MODULES([QT_CHARTS], [${qt_lib_prefix}Charts${qt_lib_suffix} $qt_version], [CPPFLAGS="$CPPFLAGS -DQT_CHARTS_LIB"],
                      [AC_MSG_WARN([${qt_lib_prefix}Charts${qt_lib_suffix} $qt_version not found. Poll results will not display charts.])])
  ])

  QT_INCLUDES="$QT_CORE_CFLAGS $QT_GUI_CFLAGS $QT_WIDGETS_CFLAGS $QT_NETWORK_CFLAGS $QT_CONCURRENT_CFLAGS $QT_CHARTS_CFLAGS"
  QT_LIBS="$QT_CORE_LIBS $QT_GUI_LIBS $QT_WIDGETS_LIBS $QT_NETWORK_LIBS $QT_CONCURRENT_LIBS $QT_CHARTS_LIBS"
