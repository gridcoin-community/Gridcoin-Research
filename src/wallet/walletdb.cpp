             ...
             CWalletScanState &wss, string& strType, string& strErr) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    try {
        ...
        else if (strType == "tx")
        {
            uint256 hash;
            ssKey >> hash;
            // Deserialize into a temporary first to avoid leaving a partial
            // CWalletTx in mapWallet if deserialization throws or validation fails.
            CWalletTx wtx_tmp;
            ssValue >> wtx_tmp;
            CValidationState wtx_state;
            if (!CheckTransaction(wtx_tmp, wtx_state) || (wtx_tmp.GetHash() != hash))
            {
                // Invalid tx - do NOT insert into mapWallet
                return false;
            }
            // Valid - move into mapWallet and get a reference for further processing
...
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    ...
    if (result == 0)
        LogPrintf("Renamed %s to %s", filename, newFilename);
    else
    {
        LogPrintf("Failed to rename %s to %s", filename, newFilename);
        return false;
    }
    ...
    {
        LogPrintf("Salvage(aggressive) found no records in %s.", newFilename);
        return false;
    }
    LogPrintf("Salvage(aggressive) found %" PRIszu " records", salvagedData.size());
    ...
    if (ret > 0)
    {
        LogPrintf("Cannot create database file %s", filename);
        return false;
    }
    CWallet dummyWallet;
    ...
    for (auto &row : salvagedData)
    {
        if (fOnlyKeys)
        {
            ...
            if (!fReadOK)
            {
                LogPrintf("WARNING: CWalletDB::Recover skipping %s: %s", strType, strErr);
                continue;
            }
        }
