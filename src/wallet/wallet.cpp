    ...
    if (conf->m_confirmed_block_height < 0) {
        ...
        if (ReadBlockFromDisk(block, pindex, Params().GetConsensus())) {
            ...
            if (vtx_index >= 0) {
                ...
                wtx.m_state = TxStateConfirmed(conf->m_confirmed_block_hash, pindex->nHeight, vtx_index);
                                       ...
                                       bool fUpdate)
{
    ...
    if (fInsertedNew) {
        ...
        if (fUpdate) {
            LogStateTransition(wtx.m_state, state);
            wtx.m_state = state;
...
bool CWallet::CommitTransaction(CWalletTx& wtxNew, CReserveKey& reservekey)
{
    ...
    }
    {
        ...
        LogPrint(BCLog::LogFlags::VERBOSE, "CommitTransaction:\n%s", wtxNew.ToString());
        {
            ...
            wtxNew.m_state = TxStateInMempool{};
...
bool CWallet::AbandonTransaction(const uint256& txid)
{
    ...
    while (!todo.empty()) {
        ...
        cur_wtx.m_state = TxStateInactive{true};
        cur_wtx.SyncLegacyFromState();
        cur_wtx.MarkDirty();
