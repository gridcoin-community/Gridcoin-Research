    ...
    ui(new Ui::DiagnosticsDialog)
{
    ...
    connect(&watcherClk, SIGNAL(finished()), this, SLOT(clkFinished()));
    connect(&watcherTCP, SIGNAL(finished()), this, SLOT(TCPFinished()));
}
...
int DiagnosticsDialog::VarifyClock(DiagnosticsDialog *dialog) {
    QHostInfo info = QHostInfo::fromName("pool.ntp.org");
    QUdpSocket *socket = new QUdpSocket(dialog);
    time_t ntpTime;
    ...
    socket->writeDatagram(sendBuffer, info.addresses().first(), 123);
...
int DiagnosticsDialog::VarifyTCPPort(DiagnosticsDialog *dialog) {
    QTcpSocket *socket = new QTcpSocket(dialog);
    socket->connectToHost("london.grcnode.co.uk", 32749);
...
void DiagnosticsDialog::on_testBtn_clicked() {
    ...
    this->repaint();
    QFuture<int> futureClk = QtConcurrent::run(DiagnosticsDialog::VarifyClock, this);
    watcherClk.setFuture(futureClk);
    //addnode
    ...
    this->repaint();
    QFuture<int> futureTCP = QtConcurrent::run(DiagnosticsDialog::VarifyTCPPort, this);
    watcherTCP.setFuture(futureTCP);

}

void DiagnosticsDialog::clkFinished() {
    if(watcherClk.future().result())
...
//void DiagnosticsDialog::runTests(DiagnosticsDialog *dialog) {
//    Ui::DiagnosticsDialog *ui = dialog->getUi();
...
//    ui->findCPIDReaultLbl->setText("Testing...");
//    dialog->repaint();
//    result = dialog->FindCPID();
//    if(result == 1)
//        ui->findCPIDReaultLbl->setText(QString::fromStdString("Passed CPID: " + dialog->KeyValue("cpid")));
//    else
//        ui->findCPIDReaultLbl->setText("Failed (Is CPID in gridcoinresearch.conf?)");
//    //cpid valid
//    ui->varifyCPIDValidResultLbl->setText("Testing...");
//    dialog->repaint();
//    result = dialog->VarifyIsCPIDValid();
//    if(result == 1)
//        ui->varifyCPIDValidResultLbl->setText("Passed");
//    else
//        ui->varifyCPIDValidResultLbl->setText("Failed (BOINC CPID and gridcoinresearch.conf CPID do not match)");
...
//    ui->varifyCPIDHasRACResultLbl->setText("Testing...");
//    dialog->repaint();
//    result = dialog->VarifyCPIDHasRAC();
//    if(result > 0)
//        ui->varifyCPIDHasRACResultLbl->setText(QString::fromStdString("Passed RAC: " + std::to_string(result)));
//    else
//        ui->varifyCPIDHasRACResultLbl->setText("Failed");
...
//    ui->varifyCPIDIsInNNResultLbl->setText("Testing...");
//    dialog->repaint();
//    result = dialog->VerifyCPIDIsInNeuralNetwork();
//    if(result == 1)
//        ui->varifyCPIDIsInNNResultLbl->setText("Passed");
//    else if (result == -1)
//        ui->varifyCPIDIsInNNResultLbl->setText("Beacon data empty, sync wallet");
...
//    ui->varifyWalletIsSyncedResultLbl->setText("Testing...");
//    dialog->repaint();
//    result = dialog->VarifyWalletIsSynced();
//    if(result == 1)
//        ui->varifyWalletIsSyncedResultLbl->setText("Passed");
//    else if (result == -1)
//        ui->varifyWalletIsSyncedResultLbl->setText("Sync data empty");
