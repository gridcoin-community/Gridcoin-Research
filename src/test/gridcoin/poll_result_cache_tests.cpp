...
// ---------------------------------------------------------------------------
// Active polls: reusable only while the tip is exactly the one tallied against
// (the active-vote-weight range ends at the tip and grows every block).
// ---------------------------------------------------------------------------

BOOST_AUTO_TEST_CASE(active_poll_reusable_when_tip_unchanged)
{
    BOOST_CHECK(GRC::PollResultReusable(/*finished=*/false,
...
BOOST_AUTO_TEST_CASE(active_poll_not_reusable_when_tip_advances)
{
    ...
    BOOST_CHECK(!GRC::PollResultReusable(false, TIP_A, 100, TIP_B, 101));
}
...
BOOST_AUTO_TEST_CASE(active_poll_not_reusable_on_same_height_reorg)
{
    ...
    BOOST_CHECK(!GRC::PollResultReusable(false, TIP_A, 100, TIP_B, 100));
}

// ---------------------------------------------------------------------------
// Closed polls: immutable while the tip only advances; reusable exactly while
// the current height has not dropped below the height they were tallied at.
// ---------------------------------------------------------------------------

BOOST_AUTO_TEST_CASE(closed_poll_reusable_when_tip_unchanged)
{
    BOOST_CHECK(GRC::PollResultReusable(/*finished=*/true, TIP_A, 100, TIP_A, 100));
}
...
BOOST_AUTO_TEST_CASE(closed_poll_reusable_when_tip_advances)
{
    ...
    BOOST_CHECK(GRC::PollResultReusable(true, TIP_A, 100, TIP_B, 250));
}
...
BOOST_AUTO_TEST_CASE(closed_poll_reusable_on_same_height_reorg)
{
    ...
    BOOST_CHECK(GRC::PollResultReusable(true, TIP_A, 100, TIP_B, 100));
}
...
BOOST_AUTO_TEST_CASE(closed_poll_not_reusable_on_backward_reorg)
{
    ...
    BOOST_CHECK(!GRC::PollResultReusable(true, TIP_A, 100, TIP_B, 99));
}
