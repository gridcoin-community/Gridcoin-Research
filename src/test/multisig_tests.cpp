...
BOOST_AUTO_TEST_CASE(multisig_verify)
{
    ...
    BOOST_CHECK(VerifyScript(s, a_and_b, flags, CTransaction(txTo[0]), 0));

    for (int i = 0; i < 4; i++)
    {
        keys.assign(1,key[i]);
        s = sign_multisig(a_and_b, keys, CTransaction(txTo[0]), 0);
        BOOST_CHECK_MESSAGE(!VerifyScript(s, a_and_b, flags, CTransaction(txTo[0]), 0), strprintf("a&b 1: %d", i));

        keys.assign(1,key[1]);
        keys.push_back(key[i]);
        s = sign_multisig(a_and_b, keys, CTransaction(txTo[0]), 0);
        BOOST_CHECK_MESSAGE(!VerifyScript(s, a_and_b, flags, CTransaction(txTo[0]), 0), strprintf("a&b 2: %d", i));
    }
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (i == 0 || i == 1)
            BOOST_CHECK_MESSAGE(VerifyScript(s, a_or_b, flags, CTransaction(txTo[1]), 0), strprintf("a|b: %d", i));
        else
            BOOST_CHECK_MESSAGE(!VerifyScript(s, a_or_b, flags, CTransaction(txTo[1]), 0), strprintf("a|b: %d", i));
    }
    s.clear();
    s << OP_0 << OP_0;
    BOOST_CHECK(!VerifyScript(s, a_or_b, flags, CTransaction(txTo[1]), 0));
    s.clear();
    s << OP_0 << OP_1;
    BOOST_CHECK(!VerifyScript(s, a_or_b, flags, CTransaction(txTo[1]), 0));
    ...
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
        {
            ...
            if (i < j && i < 3 && j < 3)
                BOOST_CHECK_MESSAGE(VerifyScript(s, escrow, flags, CTransaction(txTo[2]), 0), strprintf("escrow 1: %d %d", i, j));
            else
                BOOST_CHECK_MESSAGE(!VerifyScript(s, escrow, flags, CTransaction(txTo[2]), 0), strprintf("escrow 2: %d %d", i, j));
        }
}
...
BOOST_AUTO_TEST_CASE(multisig_IsStandard)
{
    ...
    CScript a_and_b;
    a_and_b << OP_2 << key[0].GetPubKey() << key[1].GetPubKey() << OP_2 << OP_CHECKMULTISIG;
    BOOST_CHECK(::IsStandard(a_and_b, txtype));

    CScript a_or_b;
    a_or_b  << OP_1 << key[0].GetPubKey() << key[1].GetPubKey() << OP_2 << OP_CHECKMULTISIG;
    BOOST_CHECK(::IsStandard(a_or_b, txtype));

    CScript escrow;
    escrow << OP_2 << key[0].GetPubKey() << key[1].GetPubKey() << key[2].GetPubKey() << OP_3 << OP_CHECKMULTISIG;
    BOOST_CHECK(::IsStandard(escrow, txtype));

    CScript one_of_four;
    one_of_four << OP_1 << key[0].GetPubKey() << key[1].GetPubKey() << key[2].GetPubKey() << key[3].GetPubKey() << OP_4 << OP_CHECKMULTISIG;
    BOOST_CHECK(!::IsStandard(one_of_four, txtype));
    ...
    for (int i = 0; i < 6; i++)
        BOOST_CHECK(!::IsStandard(malformed[i], txtype));
}
