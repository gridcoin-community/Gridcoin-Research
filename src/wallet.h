...
public:
    ...
    bool SelectCoinsForStaking(unsigned int nSpendTime, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, std::string& sError, bool fMiner = false) const;
    ...
    (
        ...
        if (fRead)
        {
            ...
            if (mapValue.count("spent"))
                for (auto const& c :  pthis->mapValue["spent"])
