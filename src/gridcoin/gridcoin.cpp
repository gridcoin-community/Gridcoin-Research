...
void InitializeScraper(ThreadHandlerPtr threads)
{
    ...
    if (Params().IsMockableChain()) {
        LogPrintf("Gridcoin: scraper and subscriber threads suppressed under -regtest");
        // SCAFFOLD (Phase 2B.3 / 2B.4): under -regtest, researcher flows
        // need a synthetic beacon + RSA trust-anchor injection so v3
        // ownership-proof verification can run without scrapers. Both pieces
        // require a checked-in deterministic test RSA keypair generated by an
        // offline contrib/devtools/gen-regtest-keys.py script. Master plan
        // flags both as needing maintainer signoff (regtest-only test RSA key
        // in the trust-anchor list; gate must be IsMockableChain(), never
        // IsTestChain()). Deferred until that signoff lands.
        //
        // When wiring this up, the integration points are:
        //   - RegtestSeedProjectKey() — pre-populate
        //     ScraperConsensus::mProjectPublicKeys with the test RSA pubkey
        //     keyed by a fixed regtest project URL
        //   - RegtestInitBeacon() — derive deterministic Cpid + secp256k1
        //     CKey, sign an OwnershipProof with the test RSA priv key, wrap
        //     in a CTransaction using a premine UTXO as input, route through
        //     IContractHandler::Add() into the genesis-block coinbase
        //     contracts list (keeps BeaconRegistry::Add() private)
        //   - A regtest-only "skip scraper convergence" promotion of pending
        //     v3 beacons to ACTIVE — scrapers being off makes the normal
        //     promotion path unreachable
        //
        // None of the above blocks PR #2955 testing (pool registry is
        // master-key + operator-key, no beacons / CPIDs).
    } else if (gArgs.GetBoolArg("-scraper", false)) {
