...
//!
//! \brief Contains a BOINC proof-of-account-ownership attestation from a
//! project that supports the ownership proof server extension.
//!
//! The proof binds a BOINC account ID to a Gridcoin beacon public key via an
//! RSA-SHA512 signature generated by the BOINC project server. The signed
//! message format is: "{account_id} {beacon_public_key_hex}".
//!
struct OwnershipProof
{
    ...
    //!
    //! \brief Determine whether the proof contains no data.
    //!
    bool Empty() const
    {
        return m_master_url.empty()
            && m_account_id == 0
            && m_rsa_signature.empty();
    }
...
public:
    ...
    //!
    //! \brief BOINC proof-of-account-ownership data (v3 payloads only).
    //!
    OwnershipProof m_ownership_proof;
    ...
    //!
    //! \brief Initialize a v2 beacon payload for the specified CPID.
    //!
    //! \param cpid   Identifies the researcher that advertised the beacon.
    //! \param beacon Contains the beacon public key.
    //!
    ...
    //!
    //! \brief Initialize a v3 beacon payload with an ownership proof.
    //!
    //! \param cpid   Identifies the researcher that advertised the beacon.
    //! \param beacon Contains the beacon public key.
    //! \param proof  BOINC proof-of-account-ownership data.
    //!
    BeaconPayload(const Cpid& cpid, Beacon beacon, OwnershipProof proof);
    ...
    bool WellFormed(const ContractAction action) const override
    {
        ...
        if (m_version >= 3 && action == ContractAction::ADD) {
            // v3 ADD payloads must contain a non-empty ownership proof with
            // plausible field values:
            if (m_ownership_proof.Empty()) return false;
            if (m_ownership_proof.m_master_url.empty()) return false;
            if (m_ownership_proof.m_account_id == 0) return false;
            if (m_ownership_proof.m_rsa_signature.size() < 64
                || m_ownership_proof.m_rsa_signature.size() > 1024)
            {
                return false;
            }
        }
        ...
        const ContractAction contract_action)
    {
        ...
        // v3 ownership proof is serialized before the signature exclusion
        // so that it is included in the hash (binds proof to ECDSA wallet
        // signature):
        if (m_version >= 3) {
            READWRITE(m_ownership_proof);
        }
...
public:
    ...
    //!
    //! \param key_id The key ID of the pending beacon.
    //!
